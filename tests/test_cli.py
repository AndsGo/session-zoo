...
def test_init_creates_config_dir(tmp_path):
    with patch("session_zoom.cli._config_dir", return_value=tmp_path / ".session-zoon"):
...
def test_list_empty(tmp_path):
    ...
    config_dir.mkdir()
    with patch("session_zoom.cli._config_dir", return_value=config_dir):
        with patch("session_zoom.cli._get_db") as mock_db:
...
def test_import_finds_sessions(tmp_path, sample_claude_session):
    ...
    config_dir.mkdir()
    with patch("session_zoom.cli._config_dir", return_value=config_dir), \
         patch("session_zoom.cli._claude_dir", return_value=sample_claude_session / ".claude"):
